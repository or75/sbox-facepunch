...
namespace Sandbox
{
	public partial class GlassShard
	{
		...
		private Mesh CreateMeshForShard( RenderData renderData )
		{
			...
			var panelScale = new Vector2( 1.0f / Desc.PanelSize.x, 1.0f / Desc.PanelSize.y );
			...
			for ( int i = 0; i < renderData.TotalShardVertices; i++ )
			{
				...
				Vector2 uv = renderData.VertexPositions[i];
				uv *= panelScale;
...

			{
				ComputeTriangleNormalAndTangent( out var normalSideA, out var tangentSideA,
					...
					vertices[1].TexCoord0, vertices[0].TexCoord0, vertices[2].TexCoord0 );
				...
				ComputeTriangleNormalAndTangent( out var normalSideB, out var tangentSideB,
					...
					vertices[renderData.FaceVertexCount].TexCoord0, vertices[renderData.FaceVertexCount + 1].TexCoord0, vertices[renderData.FaceVertexCount + 2].TexCoord0 );
			...
			for ( int i = 0; i < renderData.EdgeQuadCount; i++ )
			{
...

				{
					ComputeTriangleNormalAndTangent( out var faceNormal, out var faceTangent,
						...
						vertices[vertexOffset + 2].TexCoord0, vertices[vertexOffset + 1].TexCoord0, vertices[vertexOffset].TexCoord0 );
			...
			if ( ParentPanel.IsValid() )
			{
				...
				if ( ParentPanel.IsBroken && !string.IsNullOrEmpty( ParentPanel.BrokenMaterial ) ) mat = ParentPanel.BrokenMaterial;
			}
