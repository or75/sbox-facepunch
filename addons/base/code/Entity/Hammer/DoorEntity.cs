...
namespace Sandbox
{
	...
	public class ModelDoorSounds
	{
		...
		/// </summary>
		[JsonPropertyName( "open_sound" ), DisplayName( "Start opening sound" )]
		public string OpenSound { get; set; }

		/// <summary>
		/// Sound to play when the door starts to close.
		/// </summary>
		[JsonPropertyName( "close_sound" ), DisplayName( "Start closing sound" )]
	...
	/// </summary>
	[Library( "ent_door" )]
	[Hammer.Model( Archetypes = ModelArchetype.animated_model )]
	...
	public partial class DoorEntity : KeyframeEntity, IUse
	{
		...
		public override void Spawn()
		{
			...
			{
				if ( MoveDirType == DoorMoveType.Moving ) { LocalPosition = PositionA.LerpTo( PositionB, InitialPosition / 100.0f ); }
				else if ( MoveDirType == DoorMoveType.Rotating ) { LocalRotation = Rotation.Lerp( RotationA, RotationB, InitialPosition / 100.0f ); }
		...
		{
			SetAnimBool( "open", open );
			SetAnimBool( "close", !open );
		}
		...
		protected override void OnAnimGraphCreated()
		{
			...
			if ( InitialPosition > 0 )
			{
				SetAnimFloat( "initial_position", InitialPosition / 100.0f );
			}
		}
...
#region Breakable
		...
		public void Open( Entity activator = null )
		{
			...
			if ( activator != null && MoveDirType == DoorMoveType.Rotating && OpenAwayFromPlayer )
			{
